...
void UART6_init(void){
    ...
    ecPinmap(PC_6, &GPIO_TX, &pinTX);
    ecPinmap(PC_7, &GPIO_RX, &pinRX);
    ...
    GPIO_otype(PC_6, 0);
    GPIO_pupd(PC_6, 0);
    GPIO_ospeed(PC_6, EC_HIGH);

    GPIO_otype(PC_7, 0);
    GPIO_pupd(PC_7, 0);
    GPIO_ospeed(PC_7, EC_HIGH);

    // 🔥 AFR 클리어 후 AF8 설정 (중요)
    if(pinTX < 8){
        GPIO_TX->AFR[0] &= ~(0xF << (4*pinTX));
        GPIO_TX->AFR[0] |=  (8   << (4*pinTX));
    } else {
        GPIO_TX->AFR[1] &= ~(0xF << (4*(pinTX-8)));
        GPIO_TX->AFR[1] |=  (8   << (4*(pinTX-8)));
    }

    if(pinRX < 8){
        GPIO_RX->AFR[0] &= ~(0xF << (4*pinRX));
        GPIO_RX->AFR[0] |=  (8   << (4*pinRX));
    } else {
        GPIO_RX->AFR[1] &= ~(0xF << (4*(pinRX-8)));
        GPIO_RX->AFR[1] |=  (8   << (4*(pinRX-8)));
    }
    ...
    // 8N1, oversampling x16
    USART6->CR1 &= ~(USART_CR1_M | USART_CR1_PCE | USART_CR1_OVER8);
    USART6->CR2 &= ~(3 << 12);

    // Baudrate
    UART_baud(USART6, 9600);

    // TX / RX enable
    USART6->CR1 |= (USART_CR1_TE | USART_CR1_RE);

    // ⚠️ 테스트용: 인터럽트 절대 사용 안 함
...
// FIXED: USART6 RXNE wrapper 추가 (링커 에러 해결)
uint32_t is_USART6_RXNE(void){          // FIXED: USART6 RXNE 체크
