...
int A_dutystate_L = 2;
int A_dutystate_R = 2;
volatile float m_out_put_duty[4] = {0.3, 0.5, 0.75, 1};
...
void Steer(volatile uint8_t single)
{
  ...
  { check(h);
    out("Arrow: LEFT\r\n");  
    PWM_duty(DC_PWMA,1- blank_out_put_duty[abs(h)-1]);
  ...
  { check(h);
    ...
    PWM_duty(DC_PWMB,blank_out_put_duty[h-1]);
  }
  if(h==0)
  {
    check(h);
...
static void print_status_every_1s(void){
 ...
 char buffer2[80];// 문자열 버퍼
  ...
  sprintf(buffer2, "Lspeed=%lu Rspeed=%lu\r\n", (unsigned long)(1- blank_out_put_duty[A_dutystate_L])*100, (unsigned long)(blank_out_put_duty[A_dutystate_R])*100);
  // 블루투스로 출력 (USART1)
