/**
******************************************************************************
* @author  SSSLAB
* @Mod		 2024-8-23 by YKKIM  	
* @brief   Embedded Controller:  Tutorial TU_TimerInterrupt_student
* 
******************************************************************************
*/
#include "stm32f411xe.h"
#include "ecGPIO2.h"
#include "ecRCC2.h"


#define LED_pin PA_5
uint32_t count = 0;

void TIM2_init_tutorial();
void setup(void);
int main(void) { 
	// Initialiization --------------------------------------------------------
	setup();
	// Inifinite Loop ----------------------------------------------------------
	while(1){}
}
// Initialiization 
void setup(void)
{	
	RCC_PLL_init();                       // System Clock = 84MHz
	GPIO_init(LED_pin, OUTPUT);    // calls RCC_GPIOA_enable()	
	TIM2_init_tutorial();
}
// YOUR CODE GOES HERE
void TIM2_init_tutorial(){
	TIM_TypeDef* timerx;
	timerx = TIM2;
	RCC->APB1ENR |= RCC_APB1ENR_TIM2EN;
	timerx->PSC =84 ;			// Timer counter clock: 1MHz(1us)
	timerx->ARR =999999; 		// Set auto reload register to maximum (count up to 65535)
	timerx->DIER |=(1<<0);            // Enable Interrupt
	timerx->CR1 |=(1<<0);          // Enable counter
	NVIC_SetPriority(TIM2_IRQn, 2);       	// TIM2_IRQHandler Set priority as 2
	NVIC_EnableIRQ(TIM2_IRQn);			// TIM2_IRQHandler Enable
}
void GPIO_toggle(PinName_t pinName){
	GPIO_TypeDef *Port;
	unsigned int pin;
	ecPinmap(pinName,&Port,&pin);
	 Port->ODR ^= (1UL << pin); }
// YOUR CODE GOES HERE
void TIM2_IRQHandler(void){
	if((TIM2->SR & TIM_SR_UIF) ==TIM_SR_UIF      ){ // update interrupt flag
		//Create the code to toggle LED by 1000ms
		 GPIO_toggle(LED_pin); 
		TIM2->SR &= ~TIM_SR_UIF;              // clear by writing 0
	}
}
